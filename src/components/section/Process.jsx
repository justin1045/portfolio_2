          ...
          {steps.map((step, index) => (
            <RevealOnScroll key={index} animation="fade-up" delay={index * 0.1}>
              <div className={`relative flex flex-col md:flex-row items-start md:items-center gap-6 md:gap-12 ${index % 2 === 0 ? 'md:flex-row-reverse' : ''} group`}>
                ...
                {/* Timeline Node */}
                <div className="absolute left-[8px] md:left-1/2 md:-translate-x-1/2 top-4 md:top-1/2 md:-translate-y-1/2 w-6 h-6 bg-[var(--bg-primary)] border border-[var(--border-accent)] rounded-full z-10 flex items-center justify-center transition-transform group-hover:scale-125 duration-300">
                ...
                <div className={`w-full md:w-1/2 pl-12 md:pl-0 ${index % 2 === 0 ? 'md:text-left' : 'md:text-right'}`}>
                  <div className="bg-[var(--bg-card)] border border-[var(--border-subtle)] p-5 sm:p-8 rounded-2xl hover:bg-[var(--bg-card-hover)] hover:border-[var(--border-default)] transition-all duration-300 relative overflow-hidden backdrop-blur-xl">
                    {/* Hover glow */}
